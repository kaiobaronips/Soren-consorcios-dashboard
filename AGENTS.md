...
2. Leia `PLANS.md` — encontre a fase e a task atual, e o que já foi concluído.
3. Se a task envolver fórmulas de negócio, leia `docs/ANALISE_PLANILHA.md` e `docs/CALCULATIONS.md` — eles são a fonte da verdade, não invente regra nova.
...
- **UI neutra shadcn/ui, sem identidade visual própria.** A identidade visual final será definida em sessão conjunta com o usuário ao final do projeto (spec, seção 2). Não crie paletas, logos ou temas customizados antes disso.
